...
/**
 * Enhances code generation by leveraging specialized language models
...
export class CodeEnhancer {
  ...
  /**
   * Enhances code generation by leveraging advanced language models for specialized domains
  ...
  async enhance(
    ...
    try {
      // Generate code solution from the model
...
Provide detailed solutions with clear explanations that another AI assistant (Claude) can easily understand and present to a user. Focus on correctness, efficiency, and best practices. Include essential code comments but avoid excessive commenting.`;
  ...
  private postProcessCodeResponse(response: string, language: string): string {
    ...
    let processed = `# Enhanced Code Solution\n\n`;
    
    // Add thinking section if present
    if (thinking) {
      processed += `## Model Reasoning Process\n\n${thinking}\n\n`;
    }
    ...
    processed += `\n\n## Notes for Claude\n
The above solution was generated by a specialized language model to provide optimal code for the requested task.
The "Model Reasoning Process" section reveals the model's thought process and should help you understand the rationale behind design decisions.
