...
/**
 * Verifies code solutions using specialized language models
...
export class VerificationEnhancer {
  ...
  /**
   * Verifies a code solution using specialized language models
  ...
  async verify(
    ...
    try {
      // Generate verification analysis from the selected model
...
Focus on critical issues first, followed by optimization suggestions. Be specific about problems and provide concrete solutions with code examples where helpful.`;
  ...
  private postProcessVerificationResponse(response: string): string {
    ...
    let processed = `# Code Verification Analysis\n\n`;
    ...
    processed += `\n\n## Notes for Claude\n
The above analysis was generated by a specialized language model to verify the code solution.
